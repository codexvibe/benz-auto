        ...
        {isSidebarOpen && (
          <div className="fixed inset-0 z-[110] flex justify-end">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-[#0a0a0a] h-full shadow-2xl border-l border-white/5 flex flex-col"
            >
              {/* Header */}
              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-black">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="font-heading text-2xl text-white uppercase tracking-wider">Mon Panier</h2>
              ...
              <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                {cart.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-heading text-white uppercase mb-2">Votre panier est vide</h3>
                      <p className="text-[#a1a1aa] font-sans text-sm">Découvrez nos produits et commencez votre shopping !</p>
                    </div>
                    <button 
                      onClick={() => setIsSidebarOpen(false)}
                      className="mt-4 px-8 py-3 bg-white text-black font-heading uppercase hover:bg-[#39ff14] transition-colors"
                    >
                      Boutique
                    </button>
                  </div>
                  ...
                  <div className="flex flex-col gap-6">
                    {cart.map((item) => (
                      <div key={item.id} className="flex gap-4 group">
                        <div className="w-20 h-20 bg-black rounded-sm border border-white/5 flex-shrink-0 flex items-center justify-center overflow-hidden">
                          <Image 
                            src={item.image} 
                            alt={item.name} 
                            width={60} 
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-1">
                            <h4 className="font-heading text-white text-lg leading-tight">{item.name}</h4>
                            <button 
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center border border-white/10 rounded-sm">
                              ...
                              <span className="px-3 text-white font-sans font-bold text-sm border-x border-white/10">
                                {item.quantity}
                              </span>
                              <button 
                                onClick={() => updateQuantity(item.id, item.quantity + 1)}
                ...
                <div className="p-6 bg-black border-t border-white/5">
                  <div className="flex flex-col gap-4 mb-6">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-white font-sans font-bold text-xl">{subtotal.toLocaleString()} DZD</span>
                    </div>
